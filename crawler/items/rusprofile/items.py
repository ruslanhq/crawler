...
class RusprofileItem(scrapy.Item):
    ...
    status_organization = scrapy.Field(
        output_processor=TakeFirst(), input_processor=Compose(is_active_org)
    )
    ogrn = scrapy.Field(output_processor=TakeFirst())
    ogrn_date_from = scrapy.Field(
        output_processor=TakeFirst(), input_processor=Compose(dates_extractor)
    )
    date_of_register = scrapy.Field(
        output_processor=TakeFirst(), input_processor=Compose(dates_extractor)
    )
    law_address = scrapy.Field(
        output_processor=TakeFirst(), input_processor=Compose(addr)
    )
    ...
    primary_occupation = scrapy.Field(
        ...
        input_processor=Compose(lambda t: ' '.join(t).strip())
    )
    primary_occupation_code = scrapy.Field(
        ...
        input_processor=Compose(
            lambda t: re.sub(
                '[^0-9.]', '',
                t[0] if isinstance(t, list) and t else ''
            )
        )
    )
